...
void *recvrend(void *fd) {
	...
	while(true) { 	// When using show image  : waitKey(1) < 0) {
		...
		if(local_frame_obj.frame_id == UINT_MAX){
			err = write(controlPipe[1], &local_frame_obj.frame_id, sizeof(unsigned int));
			if (err < 0){
		...
		err = read(sockfd, &local_frame_obj.time_till_send, sizeof(double));
		if (err < 0){
		...
		err = read(sockfd, &local_frame_obj.time_after_send, sizeof(double));
		if (err < 0){
			perror("ERROR writing to socket");
			close(sockfd);
			exit(1);
...
*/			
		...
		cout << " , " << local_frame_obj.frame_id << ", " << local_frame_obj.correct_model << ", " << local_frame_obj.used_model << ", " << time_spent*1000 << ", " << local_frame_obj.time_till_send << ", " << local_frame_obj.time_after_send << ", " << local_frame_obj.detection_time.count()*1000 <<  endl;
...
void *capsend(void *fd) {
	...
	if(pid < 0){
		...
		while(true) {
			...
			auto end = std::chrono::system_clock::now();
			std::chrono::duration<double> spent = end - local_frame_obj.start;
			double time_till_send = spent.count()*1000;
			err = write(sockfd, &time_till_send, sizeof(double));
			if (err < 0){
				perror("ERROR writing to socket");
				close(sockfd);
				exit(1);
			...
			double time_after_send = spent.count()*1000;
			err = write(sockfd, &time_after_send, sizeof(double));
			if (err < 0){
				perror("ERROR writing to socket");
				close(sockfd);
				exit(1);
		...
		unsigned int final_frame_signal = UINT_MAX;
		err = write(sockfd, &final_frame_signal, sizeof(unsigned int));
		if (err < 0){
			perror("ERROR writing to socket");
			close(sockfd);
			exit(1);
...
int main(int argc, char *argv[]) {
	...
	pthread_mutex_destroy(&modelMutex);
